...
- 完成第二阶段统一验证入口
  ...
  - 完成部署收口 P0 文档与验收同步
    - `docs/部署准备与运行说明.md` 已补标准部署后的严格 smoke 验收步骤，作为上线前最小验收门槛
    - 已在当前开发机执行 `STRICT_SMOKE=1 make smoke`，覆盖登录、文档、Dashboard、交接、审计、版本上传/下载/预览、Assistant 完成态与群晖前置验收
    - `scripts/codex/smoke-local.sh` 已放宽 Assistant 默认轮询窗口，避免 OpenClaw 正常但响应稍慢时误判失败
    - `README.md` / `TASKS.md` 已移除过时的 smoke 阶段描述，统一到当前严格 smoke 口径
  - 已实际跑通 `make verify`
- 完成第三阶段自动化门禁骨架
  - 增加 `.github/workflows/verify.yml`
  - 增加 `.githooks/pre-commit` 与 `.githooks/pre-push`
    - 当前开发机的标准部署文档、严格 smoke 验收口径和账本状态已收口完成
    - 待在目标正式主机执行 `./scripts/codex/install-persistent-routing.sh` 并复验一次 `STRICT_SMOKE=1 make smoke`，确认正式环境与当前开发机结果一致
...
- 完成前端移动端壳层与响应式基础收口
  ...
  - `CodeView` 已补齐仓库摘要卡、单列指标区与 push 记录卡片化，并收口手机端横向滚动；代码模块可在手机端完成仓库切换和历史查看且页面不再左右滑动
...
- 完成 Go 后端 API 边界补测
  - 扩展 `backend-go/internal/transport/http/handlers/handlers_test.go`
  - 覆盖完整 router 装配下的公开路由、受保护路由、admin 鉴权门禁、internal worker token 分支
